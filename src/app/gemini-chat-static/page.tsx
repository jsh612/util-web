    ...
    try {
      ...
      if (response.success && response.chatId) {
        ...
        setMessages([
          {
            role: "bot",
            content: `${selectedCategory}에 대한 설정이 완료되었습니다. 채팅을 시작하세요.`,
          },
        ]);
        setTimeout(() => {
          if (chatInputRef.current) {
          ...
          {usageMetadata && (
            <div className="flex flex-col w-[200px]">
              <div className="text-lg text-sky-400 mt-1 flex flex-row justify-between">
                <span>입력 토큰 사용량:</span>
                <span>{usageMetadata.promptTokenCount?.toLocaleString()}</span>
              </div>
              <div className="text-lg text-sky-400 mt-1 flex flex-row justify-between">
                <span>응답 토큰 사용량:</span>
                <span>
